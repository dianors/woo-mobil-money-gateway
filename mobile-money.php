...
class WC_Gateway_Mobile_Money extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_description = __( 'Permits payments using Mobile Money.', 'woocommerce' );
    ...
    public function form() {
        ...
        $default_fields = array(
            'mm-user-field' => '<p class="form-row form-row-wide">
				...
				<input id="' . esc_attr( $this->id ) . '-mm-user" class="input-text" autocomplete="cc-number" autocorrect="no" autocapitalize="no" spellcheck="no" type="tel" placeholder="Nom de l\'abonné" name="user-mm-nam" />
			</p>',
            'mm-number-field' => '<p class="form-row form-row-wide">
				<label for="' . esc_attr( $this->id ) . '-mm-number">' . esc_html__( 'Numéro Mobile-Money', 'woocommerce' ) . ' <span class="required">*</span></label>
				<input id="' . esc_attr( $this->id ) . '-mm-number" class="input-text" inputmode="numeric" autocomplete="cc-number" autocorrect="no" autocapitalize="no" spellcheck="no" type="tel" placeholder="+229 &bull;&bull; &bull;&bull; &bull;&bull; &bull;&bull;" name="user-mm-num" />
			</p>',
        );
	...
	public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
        if ( $this->instructions && ! $sent_to_admin && 'mmoney' === $order->payment_method && $order->has_status( 'on-hold' ) ) {
	...
	public function process_payment( $order_id ) {
        ...
        if($name == '' && $number == ''){
            $name = strip_tags($_POST['user-mm-nam']);
            $number = strip_tags($_POST['user-mm-num']);
        }
		...
		$order->update_status( 'on-hold', __( "Awaiting Mobile Money payment.\nUser account: {$name} \nMMoney number: {$number}\n", 'woocommerce' ) );
